...
export class WaterjugriddleService {
    ...
    solveWaterJugProblem(x: number, y: number, z: number): StepDto[] | null {
        this.logger.log(
            `Received solve request with x_capacity=${x}, y_capacity=${y}, z_amount_wanted=${z}`,
        );

        if (z > Math.max(x, y)) {
            this.logger.error(
                'Invalid input values. Ensure Wanted Amount is less than or equal to the biggest water jug.',
            );
            return null;
        }
        ...
        if (z % this.calculateGreatestCommonDivisor(x, y) !== 0) {
            this.logger.error(
                'Invalid input values. Ensure Wanted Amount is divisible by the greatest common divisor of the water jugs.',
            );
            return null;
        }
        ...
        while (queue.length > 0) {
            ...
            this.logger.log(
                `Visiting state: ${stateKey} - x = ${currX}, y = ${currY}, z = ${z}`,
            );

            if (currX === z || currY === z) {
                const goalReachedStep = steps[steps.length - 1];
            ...
            const possibleStates: [number, number, string][] = [
                [x, currY, 'Fill bucket X'],
                [currX, y, 'Fill bucket Y'],
                [0, currY, 'Empty bucket X'],
                [currX, 0, 'Empty bucket Y'],
                [
                    Math.min(currX + currY, x),
                    currX + currY <= x ? 0 : currY - (x - currX),
                    'Pour bucket Y into X',
                ],
                [
                    currX + currY <= y ? 0 : currX - (y - currY),
                    Math.min(currX + currY, y),
            ...
            for (const [nextX, nextY, action] of possibleStates) {
                this.logger.log(
                    `Checking next state: x = ${nextX}, y = ${nextY}, action = ${action}`,
                );
                if (!visited.has(`${nextX}-${nextY}`)) {
                    this.logger.log(
                        `Adding next state: x = ${nextX}, y = ${nextY}, action = ${action} to the queue`,
                    );
                    queue.push([
                        nextX,
                        nextY,
                    ...
                    this.logger.log(
                        `Next state: x = ${nextX}, y = ${nextY}, action = ${action} already visited`,
                    );
                }
            }
        }
    ...
    private calculateGreatestCommonDivisor(a: number, b: number): number {
        ...
        const greatestCommonDivisor =
            b === 0 ? a : this.calculateGreatestCommonDivisor(b, a % b);
